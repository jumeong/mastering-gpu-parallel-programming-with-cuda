...
# 45. Optimizing the thread utilizations on vector reduction
## 1. 핵심 개념: 스레드당 작업량 가중

기존 벡터 리덕션에서는 스레드 수와 처리할 요소(Element)의 수를 1:1로 맞췄지만, 실제로는 절반 이상의 스레드가 아무 일도 하지 않는 **자원 유틸라이제이션(Utilization) 저하** 문제가 있었습니다. 이를 해결하기 위해 한 스레드가 두 개 이상의 요소를 처리하도록 설정하면 다음과 같은 이점이 발생합니다.

* **하드웨어 효율성 향상**: 스트리밍 멀티프로세서(SM)의 점유율(Occupancy)과 작업 활용도를 높입니다.
* **그리드 크기 감소**: 처리량은 그대로 유지하면서 필요한 블록(Block)의 수를 절반으로 줄일 수 있습니다.

## 2. 구현 방법: 2단계 프로세스

강의에서는 블록 크기는 256개 스레드로 유지하되, 각 블록이 512개의 요소를 처리하도록 수정하는 방법을 제시합니다.

* **1단계 (사전 합산)**: 512개 요소를 두 부분(각 256개)으로 나눕니다. 첫 번째 스레드가 '0번 요소'와 '256번 요소'를 더해 '0번 위치'에 저장하는 식으로, 모든 스레드가 자신의 인덱스에 블록 크기만큼 떨어진(Stride) 요소를 미리 더합니다.
* **2단계 (기존 리덕션)**: 1단계를 거치면 256개의 합산 결과가 남습니다. 여기에 기존에 사용하던 리덕션 알고리즘을 그대로 적용합니다.

> **코드 수정 포인트**:
> * 전역 메모리 인덱스 계산 시 `blockIdx.x * blockDim.x * 2`와 같이 **2를 곱해** 각 블록이 두 배의 영역을 담당하게 합니다.
> * 루프 진입 전 `input[i] += input[i + blockDim.x]` 코드를 추가하여 사전 합산을 수행합니다.
> 
> 

## 3. 성능 분석 결과

NVIDIA Nsight Compute 도구를 사용해 프로파일링한 결과는 다음과 같습니다.

* **실행 시간 단축**: 기본형(Baseline)이 283µs였던 것에 비해, 이 최적화를 적용하면 약 **100µs**까지 단축됩니다.
* **점유율(Occupancy)**: 약 **90%** 에 육박하는 높은 점유율을 기록하며 하드웨어 자원을 더 밀도 있게 사용함을 보여줍니다.

## 4. 추가 최적화 팁

* **루프 순서 변경**: Stride를 1부터 늘려가는 방식 대신, 최대값(`blockDim.x / 2`)부터 시작해 절반씩 줄여가는 방식을 테스트했습니다. (다만, 이 강의의 특정 환경에서는 100µs에서 150µs로 오히려 성능이 하락하는 모습도 보였습니다.)
* **연산 비용 최적화**: 나눗셈(`/`)이나 나머지(`%`) 연산은 매우 비쌉니다. 이를 **비트 시프트(Bit Shift, `>> 1`)** 연산으로 대체하면 약 2% 정도의 추가 성능 향상을 기대할 수 있습니다.

# 46. Optimization using shared memory and unrolling
### 1. GPU 메모리 계층 및 특징

강의 서두에서는 공유 메모리의 위치와 속도를 이해하기 위해 메모리 계층을 비교합니다.

| 메모리 유형 | 위치 | 속도(Latency) | 특징 |
| --- | --- | --- | --- |
| **레지스터 (Registers)** | SM 내부 | 가장 빠름 | 스레드 전용 최속 메모리 |
| **공유 메모리 (Shared)** | SM 내부 | 매우 빠름 (약 25-30 cycles) | **프로그래밍 가능(L1과 물리적 동일 유닛)** |
| **L1 캐시** | SM 내부 | 매우 빠름 (약 30 cycles) | 하드웨어가 자동으로 제어 |
| **L2 캐시** | GPU 전체 공유 | 중간 (약 180-200 cycles) | 모든 SM이 공유하는 캐시 레벨 |
| **전역 메모리 (Global)** | GPU 외부 | 느림 (약 300+ cycles) | 용량은 크나 지연 시간이 매우 김 |

### 2. 공유 메모리를 활용한 최적화

공유 메모리를 사용하면 전역 메모리에 대한 직접적인 접근(Read/Write) 횟수를 획기적으로 줄일 수 있습니다.

* **동작 원리**:
1. 각 블록은 공유 메모리에 공간을 할당합니다(`__shared__ float sdata[256]`).
2. 전역 메모리의 데이터를 한 번 읽어 공유 메모리에 저장합니다.
3. 이후 리덕션 연산(덧셈)은 전역 메모리가 아닌 **공유 메모리 내에서 수행**합니다.
4. 최종 결과 값 하나만 전역 메모리에 기록합니다.


* **효과**: 전역 메모리 쓰기(Write) 작업이 대폭 감소합니다(스크립트 예시: 18MB → 1MB 이하).
* **주의사항**: 공유 메모리 단독 사용만으로는 성능 향상이 드라마틱하지 않을 수 있습니다(L1 캐시 덕분에 이미 어느 정도 가속되고 있기 때문). 따라서 다른 기법과 결합해야 합니다.

### 3. 루프 언롤링(Loop Unrolling)과 결합

리덕션 단계에서 `stride`가 32보다 작아지는 순간(즉, 하나의 워프 내에서만 작업이 일어날 때) 최적화하는 기법입니다.

* **워프 동기화(Warp Synchronization)**: 워프(32개 스레드) 내에서는 명령어가 동기적으로 실행되므로, `__syncthreads()` 호출을 생략할 수 있습니다.
* **구현**: `stride`가 32 이상일 때는 `for` 루프와 동기화를 사용하고, 32 미만이 되면 별도의 인라인 함수나 장치 함수(`__device__`)를 호출하여 수동으로 덧셈을 나열(Unrolling)합니다.
* **효과**: 루프 제어 오버헤드와 불필요한 동기화 지점을 제거하여 성능을 추가로 10% 이상 향상시킵니다.

### 4. 성능 측정 결과 분석

강의에서 프로파일링 도구(Nsight Compute)를 통해 보여준 최적화 단계별 실행 시간은 다음과 같습니다.

1. **Global Memory + More work per thread**: 약 $100 \mu s$
2. **Shared Memory 적용**: 약 $103 \mu s$ (단독 적용 시 오히려 미세하게 증가할 수 있음)
3. **Shared Memory + Loop Unrolling**: 약 $90 \mu s$
4. **Block Size 최적화 (256 → 128)**: **약 $82 \mu s$ (최종 승자)**

### 5. 주요 팁 및 결론

* **뱅크 충돌(Bank Conflict)**: 공유 메모리 사용 시 `stride`를 큰 값에서 작은 값으로 줄여나가는 방식을 권장합니다. 반대로 할 경우 뱅크 충돌로 인해 성능이 저하될 수 있습니다.
* **시행착오(Trial and Error)**: 블록 크기(128, 256, 512 등)와 같은 하드웨어 설정값은 이론적인 계산뿐만 아니라, 실제 프로파일링을 통해 최적의 값을 찾아야 합니다.
* **템플릿(Template)**: `if` 분기문을 컴파일 타임에 결정하여 실행 시점의 워프 분기(Warp Divergence)를 줄이는 기법도 유용합니다.

# 47. Shuffle operations optimizations

## 1. 메모리 계층과 Shuffle의 필요성

GPU 성능 최적화의 핵심은 "데이터를 어디에 두고 연산하느냐"입니다.

* **레지스터(Register):** GPU에서 가장 빠른 메모리(L5)이지만, 스레드 간 데이터 공유가 불가능했습니다.
* **기존 방식:** 데이터를 공유하려면 더 느린 Shared Memory나 Global Memory에 쓰고 다시 읽어야 했습니다.
* **Shuffle 연산의 혁신:** 워프(Warp, 32개 스레드 단위) 내에서 Shared Memory를 거치지 않고 **스레드 간 레지스터 값을 직접 교환**할 수 있게 해줍니다.

---

## 2. Shuffle 기반 리덕션 구현 단계

강의에서 설명한 커널(`Kernel 06`)의 내부 로직은 다음과 같습니다.

1. **데이터 로드 및 1차 합산:** 글로벌 메모리에서 데이터를 가져와 각 스레드의 레지스터 변수(`sum`)에 할당합니다. 이때 한 스레드가 두 개의 요소를 더하게 하여 전체 로드 횟수를 줄입니다.
2. **워프 내 리덕션 (Warp Reduce):** `__shfl_down_sync` 명령어를 사용하여 워프 내부의 32개 스레드 값을 하나로 합칩니다.
* **Stride(Delta):** 16 → 8 → 4 → 2 → 1 순으로 줄어들며 인접 스레드의 값을 가져와 합산합니다.


3. **Shared Memory 활용:** 각 워프가 계산한 '부분 합(Partial Sum)'을 Shared Memory의 연속된 공간에 저장합니다.
4. **최종 합산:** Shared Memory에 저장된 각 워프의 결과값들을 첫 번째 워프가 다시 Shuffle 연산으로 합쳐, 블록당 최종 1개의 결과값을 산출합니다.

---

## 3. 성능 분석 및 결과

강사님이 강조한 이번 최적화의 핵심 지표는 **실행 시간의 비약적인 단축**입니다.

### 성능 비교 데이터

| 최적화 단계 | 실행 시간 (Execution Time) | 성능 향상도 |
| --- | --- | --- |
| **기존 최적화 최소값** | **82 $\mu$s** | 기준 |
| **Shuffle 연산 적용** | **17~18 $\mu$s** | **약 4배 (4x) 향상** |

### Nsight Compute 프로파일링 가이드

* **명령어 방식:** `gpu__time_active` 메트릭을 통해 정확한 실행 시간을 확인합니다.
* **GUI 방식:** Nsight Compute를 실행 중인 프로세스에 `Attach`하여 상세한 대역폭(Bandwidth)과 처리량(Throughput)을 분석할 수 있습니다.
* **주의사항:** Windows 환경에서 컴파일 시 실행 파일 확장자(`.exe`)를 명시해야 GUI 도구에서 정상적으로 인식됩니다.

---

## 4. 추가 최적화 팁 (Homework)

강의 마무리에서 언급된 더 높은 성능을 위한 개선 포인트들입니다.

* **산술 연산 최적화:** 비용이 많이 드는 나눗셈(`/`)이나 나머지 연산(`%`)을 비트 시프트(`>> 1`) 연산 등으로 대체하는 것을 권장합니다.
* **커널 반복 실행:** 동일한 커널을 반복 실행했을 때 캐싱 효과 등으로 인해 실행 시간이 3 $\mu$s까지 더 단축되는 것을 확인할 수 있습니다.
